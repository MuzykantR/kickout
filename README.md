...
**Управление:** `A/D` или стрелки — движение, `Space` — прыжок (удержание выше), `LShift` — спринт, `S` — присед, `C` (зажать) — обзор всего уровня, `W/S` при скольжении вдоль стены — медленнее/быстрее скольжение, `R` — рестарт с первого уровня, `Esc` — выход.

**Ассеты:** игра запускается даже без PNG-текстур и звуков — отсутствующие ресурсы заменяются цветными прямоугольниками строго фиксированного размера. Полная спецификация ассетов: [`docs/ASSETS.md`](docs/ASSETS.md).
